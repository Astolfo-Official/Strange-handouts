...
DIIS (Direct Inversion in the Iterative Subspace) 可以直接用在这个形式下引入，其核心思想是利用前几步迭代的信息，通过线性组合构造更优的 Fock 矩阵，使误差矩阵最小化。相比于简单的迭代，DIIS 可以将收敛速度从线性加速至超线性，通常能将所需迭代步数减少 50$\sim$80\%。如最经典的 DIIS 方法，定义第 $i$ 步迭代的误差矩阵：
...
\[L = \sum_{k,l} c_k c_l E_{kl} - \lambda \left(\sum_k c_k - 1\right)\]
对 $c_i$ 求偏导：
\[\frac{\partial L}{\partial c_i} = 2\sum_{k} c_k E_{ki} - \lambda = 0\]

整理得到线性方程组：
$$\begin{pmatrix}
2E_{11} & 2E_{12} & \cdots & 2E_{1n} & -1 \\
2E_{21} & 2E_{22} & \cdots & 2E_{2n} & -1 \\
\vdots & \vdots & \ddots & \vdots & \vdots \\
2E_{n1} & 2E_{n2} & \cdots & 2E_{nn} & -1 \\
-1 & -1 & \cdots & -1 & 0
\end{pmatrix}
...
\begin{center}
    \includegraphics[scale=0.65]{fig/qquantum_chemistry/mpx.png}
\end{center}
